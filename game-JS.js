...
HTML5Game = {
	...
	initData: function(){
		for(var x=0;x<=1;x++){for(var i=0; i<= (numOfBoxGame/2)-1;i++){boxIndexes.push(i);}}
			...
			if(noOfClick <= 1){
				...
				if(numOfClick >= 2){
					...
					if(clickImages[0].ImageID == clickImages[1].ImageID){
						...
						if(correctGuess >= (numOfBoxGame/2)){
